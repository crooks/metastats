...
def remailer_filename(name, addy):
    noat = addy.replace('@',".")
    filename = '%s/%s.%s.txt' % (config.type1dir, name, noat)
    filename_chfr = '%s/chfr.%s.%s.txt' % (config.type1dir, name, noat)
    filename_chto = '%s/chto.%s.%s.txt' % (config.type1dir, name, noat)
...
def gene_write_html():
    ...
    filename = "%s/%s" % (config.type1dir, config.gene_report_name)
...
def index_generate(html):
    ...
    filename = "%s/index.html" % config.type1dir
